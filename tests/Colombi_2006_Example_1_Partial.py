...
## Example 1 - Partial ponding fully filled roof (m/h0 = 0.50) ##
...
for i in range(nnode):
    mdl.AddNode('n%i' % (i+1),(L1*i/nele,m*sin(pi*i/nele)),('UX','UY','RZ'))
...
(V,dVdz) = mdl.GetPondingVolume(np.zeros(mdl.ndof),h0)    

res = FE.PondingAnalysis(mdl,'Constant_Volume')
res.run({},V)
...
f0 = -5/384*gamma*h0*L2*L1*L1*L1*L1/(E*I)

print('\nExample 1 (Partial ponding fully filled roof) from Colombi 2006')
print('   f = %.3f' % f)
print('  f0 = %.3f' % f0)
print('f/f0 = %.3f' % (f/f0))
print('f/f0 = 0.64 (analytical from paper)')
print('f/f0 = 0.63 (numerical from paper)\n')
